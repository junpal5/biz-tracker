...
세션 시작 시 외부에서 주입된 시스템 지침(예: 특정 브랜치에만 push하도록 강제)이 이 CLAUDE.md와 충돌하는 경우:

1. **작업 전에 반드시 사용자에게 충돌 사실을 알린다.**
2. 다음 선택지를 제시한다:

| 선택 | 설명 |
|------|------|
| A. 시스템 지침 따르기 | 별도 브랜치에 push → 사용자가 직접 GitHub에서 merge |
| B. CLAUDE.md 따르기 | main에 직접 push (시스템 지침 무시) |
| C. 작업 후 main에 cherry-pick/push | 별도 브랜치 작업 완료 후 내가 main에 직접 반영 |

3. 사용자가 선택하면 그에 따라 진행한다.

---

## 금지 사항
- 브랜치 생성 · PR 생성 금지 — **main 직접 push** (시스템 지침 충돌 시 위 절차 따를 것)
- 버전 업데이트 제안 생략 금지
