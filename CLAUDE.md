...
- **앱명**: 🏢 사업자 휴·폐업 조회
- **파일**: `index.html` (단일 파일 — 모든 CSS·HTML·JS 포함, 약 670줄)
...
| Step 2 | XLSX 파일 업로드 → SheetJS로 파싱 → 사업자등록번호 열 선택 드롭다운 |
| Step 3 | 조회 실행 → 100건 단위 배치 API 호출 → 프로그레스바 → 결과 테이블 표시 |
...
- **요청 body**: `{ "businesses": [{ "b_no": "0000000000" }, ...] }`
- **배치 한도**: 1회 최대 100건
